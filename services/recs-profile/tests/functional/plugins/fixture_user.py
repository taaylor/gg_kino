...
async def create_user(make_request) -> Callable[[], Coroutine[Any, Any, dict[str, Any]]]:
    async def inner() -> dict[str, Any]:
        ...
        register_status, register_body = await make_request(
            method=HTTPMethod.POST, url=register_url, data=register_data.model_dump(mode="json")
        )

        # Если регистрация вернула ошибку 400, пытаемся авторизоваться
        if register_status == HTTPStatus.BAD_REQUEST:
            ...
            login_status, login_body = await make_request(
                method=HTTPMethod.POST, url=login_url, data=login_data.model_dump(mode="json")
            )
        ...
        elif register_status == HTTPStatus.OK:
