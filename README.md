...
Для упрощения работы с проектом предоставлен Makefile, который содержит часто используемые команды. Ко всем командам можно указать атрибут `srv`, чтобы применить действие только к определённому сервису. Например:
```bash
make up srv=auth-api
```
Если `srv` не указан, команда применяется ко всем сервисам.

### 🚀 Запуск контейнеров
```bash
make up
```
Запускает все контейнеры в фоновом режиме с пересборкой образов.
...
make down-v
```
Останавливает все контейнеры и удаляет связанные тома.

### Запуск контейнеров и просмотр логов
```bash
make up-logs
```
...
make clean
```
Останавливает контейнеры и удаляет все тома.
