...
- [🎥 KINOSERVICE](#-kinoservice)
  ...
  - [🛠 Сервисы проекта](#-сервисы-проекта)
    ...
    - [🛎️ Metric Service](#️-metric-service)
      ...
      - [Аналитическое сравнение производительности MongoDB и Postgres для сервиса content-actions](#аналитическое-сравнение-производительности-mongodb-и-postgres-для-сервиса-content-actions)
    - [📈 Jaeger](#-jaeger)
    ...
    - [🪤 Notification-контекст](#-notification-контекст)
      - [Запуск сервисов контекста](#запуск-сервисов-контекста)
        - [Запуск email-sender](#запуск-email-sender)
...
#### Аналитическое сравнение производительности MongoDB и Postgres для сервиса content-actions
Аналитическое сравнение приводится в следующем [README](/docs/compare-mongodb-vs-postgres/README.md)
...
1. Рецензии на фильмы: Пишите развернутые отзывы о просмотренных фильмах, делитесь впечатлениями и обсуждайте их с сообществом.
2. Оценки фильмов: Ставьте рейтинги фильмам, чтобы влиять на их популярность и помогать другим пользователям в выборе.
3. Закладки: Сохраняйте понравившиеся фильмы в личный список, чтобы легко вернуться к ним позже.
...
### 🪤 Notification-контекст
С сервисами контекста можно ознакомиться на [диаграмме](#️-архитектурный-ландшафт).

Контекст решает следующие задачи:
1. Административная панель (**event-generator**) API для создания рассылок менеджерами проекта.
2. Сервис нотификаций (**notification-processor**) реализующий:
   1. Обработка поступающих фиксированных событий в проекте:
      1. Регистрация пользователя.
      2. Лайк на комментарий пользователя к фильму.
   2. Массовая рассылка уведомлений всем пользователям.
   3. Обогащение поступающих запросов на отправку нотификации дополнительными данными.
   4. Управление временем отправки уведомления в соответствии с таймзоной пользователя и приоритетом.
      1. Уведомления отправляются **с 9 до 20 часов** по таймзоне пользователя (по умолчанию МСК).
   5. Управление жизненным циклом уведомления.
3. Воркеры:
   1. WebSocket воркер (**websocket-sender**) - отправляет уведомления в активную сессию пользователя в приложении.
   2. Email воркер (**email-sender**) - отправляет уведомление в виде письма на email.
4. Сервис сокращения ссылок (**short-link-service**) - реализует функционал сокращения ссылок и управления жизненным циклом сокращённой ссылки.


#### Запуск сервисов контекста
1. `.env.example` переименовать в `.env`
2. Из корня проекта выполнить: `make up-notification-context`
3. Выполнить регистрацию пользователя: `http://127.0.0.1/auth/openapi`:
   1. Метод: `POST: /auth/api/v1/sessions/register`
4. В логах notification сервиса будет сообщение об отправке уведомления о регистрации на email.
5. Перейти к API сервиса нотификаций: `http://127.0.0.1/notification/openapi`
   1. Указать user_id зарегистрированного пользователя в атрибутах: `liked_by_user_id, user_id` в методе: `POST: /notification/api/v1/notifications/single-notification` и отправить запрос.
      1. В логах notification сервиса будет сообщение об отправке уведомления о лайке на комментарий пользователя к фильму.
   2. Отправить массовую рассылку, также указав `user_id` зарегистрированного пользователя: `POST /notification/api/v1/notifications/mass-notification`
      1. В логах notification сервиса будет сообщение об обработке массовой рассылки.
         1. Сервис получит user_id всех пользователей по API Auth сервиса и для каждого создаст единичное уведомление.
6. Перейти к API сервиса сокращения ссылок: `http://localhost/link/openapi`
   1. Запросить сокращение любой http ссылки в методе: `POST: /link/api/v1/link/shorten`
   2. Перейти по ссылке из атрибута ответа: `short_url`
7. Перейти в Админ панель менеджера `http://localhost/event-generator/openapi`
   1. Получить список шаблонов: `GET: /event-generator/api/v1/admin/templates`
   2. Отправить массовую нотификацию на всех пользователей: `POST: /event-generator/api/v1/notify/create-mass-notify`
8. Перейти в MailHog `http://localhost:8025/` проверить доставку писем до адресатов.
9. Протестировать доставку WebSocket уведомлений можно с по адресу `ws://localhost/ws-notification/api/v1/ws-sender`
10. Подключиться к БД и посмотреть жизненный цикл экземпляров уведомлений и коротких ссылок:
```
...
##### Запуск email-sender
  1 - регистрируетесь:
  http://localhost/auth/openapi#/%D0%A1%D0%B5%D1%81%D1%81%D0%B8%D0%B8/register_auth_api_v1_sessions_register_post

  2 - копируете user_id который к вам пришёл в ответе при регистрации
  3 - делаете запрос на то что нужно сделать уведомление:
      http://127.0.0.1/notification/openapi#/%D0%9D%D0%BE%D1%82%D0%B8%D1%84%D0%B8%D0%BA%D0%B0%D1%86%D0%B8%D0%B8/create_single_notification_notification_api_v1_notifications_single_notification_post
  4 - в логах email-sender вы увидите информацию об успешной отправке письма
  5 - в логах сервиса mailhog (эмулятор smtp сервера) вы увидите что отправка совершилась
...
└── services                        # Микросервисы приложения
    ...
    └── ws_notification-service     # Сервис для отправки уведомлений через WebSocket
        └── src                     # Исходный код WebSocket сервиса
```
...
![Архитектурный ландшафт](docs/c4.tobe.drawio.png)
