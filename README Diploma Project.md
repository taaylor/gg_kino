...
**nl-consumer** — FastAPI‑микросервис для «умного» поиска фильмов: принимает и валидирует запрос пользователя, передаёт текст в NLP/LLM‑сервис для анализа и генерации embedding‑вектора, сохраняет в БД исходный запрос, статус и результат обработки, а затем возвращает пользователю список рекомендованных фильмов.
